...
@Singleton
class ProductUpdateUseCase @Inject constructor(private val repository: ProductRepository) {
	operator fun invoke(product: Product): Flow<ResponseResult<Product>> = repository.update(product)
}